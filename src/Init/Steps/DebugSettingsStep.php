...
class DebugSettingsStep extends AbstractStep
{
    ...
    public function execute(State $state)
    {
        ...
        if(isset($state["ZEND_DEBUG"]) && $state["ZEND_DEBUG"] === true) {
            ...
            self::pregReplaceFile("/zend_gui.logVerbosity = NOTICE/", "zend_gui.logVerbosity = DEBUG", "/usr/local/zend/gui/config/zs_ui.ini");
            self::pregReplaceFile('/zend_gui.debugModeEnabled = false/', 'zend_gui.debugModeEnabled = true', "/usr/local/zend/gui/config/zs_ui.ini");
            self::pregReplaceFile('/zend_deployment.daemon.log_verbosity_level=2/', 'zend_deployment.daemon.log_verbosity_level=5', "/usr/local/zend/etc/zdd.ini");
            self::pregReplaceFile('/zend_server_daemon.log_verbosity_level=2/', 'zend_server_daemon.log_verbosity_level=5', "/usr/local/zend/etc/zsd.ini");
        }
