...
class LicenseStep extends AbstractStep
{
    ...
    public function execute(State $state)
    {
        ...
        if (is_file(self::LICENSE_FILE)) {
            ...
            if ($license == null) {
                $state->log->log(Log::WARNING, "Could not parse license file");
                $state['ZEND_LICENSE_KEY'] = "";
                $state['ZEND_LICENSE_ORDER'] = "";
            } else {
            ...
            self::pregReplaceFile("/zend\\.serial_number=.*\$/", "zend.serial_number={$state['ZEND_LICENSE_KEY']}", "/usr/local/zend/etc/conf.d/ZendGlobalDirectives.ini");
            self::pregReplaceFile("/zend\\.user_name=.*\$/", "zend.user_name={$state['ZEND_LICENSE_ORDER']}", "/usr/local/zend/etc/conf.d/ZendGlobalDirectives.ini");
